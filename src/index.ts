...
const cutTo1000 = (s: string) => s.length <= 1000 ? s : s.slice(0, 1000);
/**
 * 将数字转换为上标数字
...
[关键字2](链接本体)`,
			...
			const res = await fetch(`https://api.telegram.org/bot${env.SECRET_TELEGRAM_API_TOKEN}/sendMessage`, {
				...
				body: JSON.stringify({
					...
					text: cutTo1000(processMarkdownLinks(telegramifyMarkdown(messageTemplate(result.response.text()), 'keep'))),
